...
rules:
-only one pass
-second to last node should be removed
-input list will have 2+ elemnts
-if input has exactly 2 elements, return second node

algorithm:
-- the trick will prob be determining whether a given node is the second to last
in a single pass

maybe i can repeastedly check whether
  curr.next.next === null
  when that's true,
  prev.next = curr.next
